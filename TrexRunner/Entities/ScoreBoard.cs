...
namespace TrexRunner.Entities
{
    public class ScoreBoard : IGameEntity
    {
        ...
        public double Score { 
            get => _score; 
            set => _score = Math.Max(0, Math.Min(MAX_SCORE, value));  // dont exceed the max possible score and the lesser cannot be negative
        }
        ...
        public void Draw(SpriteBatch spriteBatch, GameTime gameTime)
        {
            ...
            if (HasHighScore)
            {
                spriteBatch.Draw(_texture, new Vector2(Position.X - HI_TEXT_MARGIN, Position.Y), new Rectangle(TEXTURE_COORDS_HI_X, TEXTURE_COORDS_HI_Y, TEXTURE_COORDS_HI_WIDTH, TEXTURE_COORDS_HI_HEIGHT), TrexRunnerGame.GAME_SCENE_COLOR);
                DrawScore(spriteBatch, HighScore, Position.X);

            }
            // draw score on each odd frame within the animation
            if (!_isPlayingFlashAnimation || (int)(_flashAnimationTime / FLASH_ANIMATION_FRAME_LENGTH) % 2 != 0)
            {
                int score = !_isPlayingFlashAnimation ? DisplayScore : (DisplayScore / 100 * 100); // round to next 100 during blinking but keep counting score
        ...
        public void DrawScore(SpriteBatch spriteBatch, int score, float startPosX)
        {
            ...
            foreach (int digit in scoreDigits)
            {
                ...
                spriteBatch.Draw(_texture, screenPos, textureCoords, TrexRunnerGame.GAME_SCENE_COLOR);
        ...
        {
            if (digit < 0 || digit > 9)
                throw new ArgumentOutOfRangeException("digit", "Ayayai, value of digit must be between 0 and 9.");
